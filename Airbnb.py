# Make sure this is installed: pip install streamlit-option-menu
...
#  Main title
st.markdown("<h1 style='color:#FF5A5F;'>🏠 Airbnb Data Analysis</h1>",
...
@st.cache_data
def load_data():
    df = pd.read_csv(r'C:\Sudhakar\Projects\Airbnb Analysis\Airbnb.csv')
    return df
...
if option == 'Home':
    ...
    st.image(Image.open(
        r"C:\Sudhakar\Projects\Airbnb Analysis\Dataset & Documents\img.JPG"), width=500)

    st.markdown("<h1 style='color:#32cd32;'>About Airbnb</h1>",
                unsafe_allow_html=True)
    st.write("")
    st.write('''Airbnb is an online marketplace that connects people who want to rent out 
              ...
              earn some income from their property.Guests often find that Airbnb rentals are cheaper and homier than hotels.''')
    ...
    st.markdown("<h1 style='color:#32cd32;'>Background of Airbnb</h1>",
                unsafe_allow_html=True)
    st.write("")
...
if option == 'Data Analysis':
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(
        ["PRICE ANALYSIS", "AVAILABILITY ANALYSIS", "LOCATION ANALYSIS", "GEOSPATIAL ANALYSIS", "TOP CHAERTS"])

    with tab1:
        st.markdown(
            "<h1 style='color:#0000cd; font-size: 36px;'>Price Analysis</h1>", unsafe_allow_html=True)
        ...
        with col1:
            ...
            room_type = st.selectbox(
                "Select the Room Type", df_1["room_type"].unique())

            df_2 = df_1[df_1["room_type"] == room_type]
            df_2.reset_index(drop=True, inplace=True)

            df_bar = pd.DataFrame(df_2.groupby("property_type")[
                                  ["price", "review_scores", "number_of_reviews"]].sum())
            df_bar.reset_index(inplace=True)

            fig_bar = px.bar(df_bar, x='property_type', y="price", title="PRICE FOR PROPERTY TYPES", hover_data=[
        ...
        with col2:
            ...
            proper_type = st.selectbox(
                "Select the Property_type", df_2["property_type"].unique())

            df_3 = df_2[df_2["property_type"] == proper_type]
            df_3.reset_index(drop=True, inplace=True)

            df_pie = pd.DataFrame(df_3.groupby("host_response_time")[
                                  ["price", "bedrooms"]].sum())
        ...
        with col1:
            ...
            hostresponsetime = st.selectbox(
                "Select the host response time", df_3["host_response_time"].unique())

            df_4 = df_3[df_3["host_response_time"] == hostresponsetime]

            df_do_bar = pd.DataFrame(df_4.groupby("bed_type")[
        ...
        with col2:
            ...
            df_do_bar_2 = pd.DataFrame(df_4.groupby("bed_type")[
    ...
    with tab2:
        def data_load():
            availability_df = pd.read_csv(
                r"C:\Sudhakar\Projects\Airbnb Analysis\Airbnb.csv")
            return availability_df

        availability_df = data_load()

        st.markdown(
            "<h1 style='color:#0000cd; font-size: 36px;'>Availability Analysis</h1>", unsafe_allow_html=True)

        col1, col2 = st.columns(2)
        with col1:

            available_country = st.selectbox(
                "Select the available country", availability_df["country"].unique())

            availability_df_1 = df[df["country"] == available_country]
            availability_df_1.reset_index(drop=True, inplace=True)

            available_property_type = st.selectbox(
                "Select the Property Type", availability_df_1["property_type"].unique())

            availability_df_2 = availability_df_1[availability_df_1["property_type"]
                                                  == available_property_type]
            availability_df_2.reset_index(drop=True, inplace=True)

            df_a_sunb_30 = px.sunburst(availability_df_2, path=["room_type", "bed_type", "is_location_exact"], values="availability_30",
                                       width=600, height=500, title="Availability of next 30 days", color_discrete_sequence=px.colors.sequential.Peach_r)
        ...
        with col2:
            ...
            df_a_sunb_60 = px.sunburst(availability_df_2, path=["room_type", "bed_type", "is_location_exact"], values="availability_60",
                                       width=600, height=500, title="Availability of next 60 days", color_discrete_sequence=px.colors.sequential.Blues_r)
        ...
        with col1:
            ...
            df_a_sunb_90 = px.sunburst(availability_df_2, path=["room_type", "bed_type", "is_location_exact"], values="availability_90",
                                       width=600, height=500, title="Availability of next 90 days", color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
            st.plotly_chart(df_a_sunb_90)

        with col2:

            df_a_sunb_365 = px.sunburst(availability_df_2, path=["room_type", "bed_type", "is_location_exact"], values="availability_365",
                                        width=600, height=500, title="Availability of next 365 days", color_discrete_sequence=px.colors.sequential.Greens_r)
            st.plotly_chart(df_a_sunb_365)

        roomtype_a = st.selectbox(
            "Select the available Room Type", availability_df_2["room_type"].unique())

        availability_df_3 = availability_df_2[availability_df_2["room_type"] == roomtype_a]

        mul_bar_availability_df = pd.DataFrame(availability_df_3.groupby("host_response_time")[
    ...
    with tab3:
        ...
        st.markdown(
            "<h1 style='color:#0000cd; font-size: 36px;'>Location Analysis</h1>", unsafe_allow_html=True)
        ...
        country_location = st.selectbox(
            "Select the country_location", location_df["country"].unique())

        location_df_1 = location_df[location_df["country"] == country_location]
        location_df_1.reset_index(drop=True, inplace=True)

        proper_ty_l = st.selectbox(
            "Select the Property_type_l", location_df_1["property_type"].unique())

        location_df_2 = location_df_1[location_df_1["property_type"]
                                      == proper_ty_l]
        location_df_2.reset_index(drop=True, inplace=True)

        st.write("")

        def select_the_df(selected_value):
            if selected_value == str(location_df_2['price'].min())+' '+str('to')+' '+str(differ_max_min*0.30 + location_df_2['price'].min())+' '+str("(30% of the Value)"):

                df_val_30 = location_df_2[location_df_2["price"] <=
                                          differ_max_min*0.30 + location_df_2['price'].min()]
                df_val_30.reset_index(drop=True, inplace=True)
                return df_val_30

            elif selected_value == str(differ_max_min*0.30 + location_df_2['price'].min())+' '+str('to')+' '+str(differ_max_min*0.60 + location_df_2['price'].min())+' '+str("(30% to 60% of the Value)"):

                df_val_60 = location_df_2[location_df_2["price"] >=
                                          differ_max_min*0.30 + location_df_2['price'].min()]
                df_val_60_1 = df_val_60[df_val_60["price"] <=
                                        differ_max_min*0.60 + location_df_2['price'].min()]
                df_val_60_1.reset_index(drop=True, inplace=True)
                return df_val_60_1

            elif selected_value == str(differ_max_min*0.60 + location_df_2['price'].min())+' '+str('to')+' '+str(location_df_2['price'].max())+' '+str("(60% to 100% of the Value)"):

                df_val_100 = location_df_2[location_df_2["price"] >=
                                           differ_max_min*0.60 + location_df_2['price'].min()]
                df_val_100.reset_index(drop=True, inplace=True)
                return df_val_100

        differ_max_min = location_df_2['price'].max(
        )-location_df_2['price'].min()

        selected_value = st.radio("Select the Price Range", [str(location_df_2['price'].min())+' '+str('to')+' '+str(differ_max_min*0.30 + location_df_2['price'].min())+' '+str("(30% of the Value)"),
                                                             str(differ_max_min*0.30 + location_df_2['price'].min())+' '+str('to')+' '+str(
            differ_max_min*0.60 + location_df_2['price'].min())+' '+str("(30% to 60% of the Value)"),
            str(differ_max_min*0.60 + location_df_2['price'].min())+' '+str('to')+' '+str(location_df_2['price'].max())+' '+str("(60% to 100% of the Value)")])

        df_selected_value = select_the_df(selected_value)

        st.dataframe(df_selected_value)

        # checking the correlation

        df_selected_value_corr = df_selected_value.drop(columns=['listing_url', 'name', 'summary', 'space', 'description',
        ...
        st.dataframe(df_selected_value_corr)

        df_selected_value_gr = pd.DataFrame(df_selected_value.groupby("accommodates")[
            ["cleaning_fee", "bedrooms", "beds", "extra_people"]].sum())
        df_selected_value_gr.reset_index(inplace=True)

        fig_1 = px.bar(df_selected_value_gr, x="accommodates", y=["cleaning_fee", "bedrooms", "beds"], title="ACCOMMODATES",
        ...
        room_ty_l = st.selectbox(
            "Select the Room Type for the location", df_selected_value["room_type"].unique())

        df_selected_value_rt = df_selected_value[df_selected_value["room_type"] == room_ty_l]

        fig_2 = px.bar(df_selected_value_rt, x=["street", "host_location", "host_neighbourhood"], y="market", title="MARKET",
        ...
        fig_3 = px.bar(df_selected_value_rt, x="government_area", y=["host_is_superhost", "host_neighbourhood", "cancellation_policy"], title="GOVERNMENT_AREA",
    ...
    with tab4:
        ...
        st.markdown(
            "<h1 style='color:#0000cd; font-size: 36px;'>Geospatial Analysis</h1>", unsafe_allow_html=True)
    ...
    with tab5:
        ...
        st.markdown(
            "<h1 style='color:#0000cd; font-size: 36px;'>Top Chart Analysis</h1>", unsafe_allow_html=True)

        country = st.selectbox("Select the Country type",
                               df["country"].unique())

        top_chart_df = df[df["country"] == country]

        top_chart_property_type = st.selectbox(
            "Select the Property type for view the chart", top_chart_df["property_type"].unique())

        top_chart_df_1 = top_chart_df[top_chart_df["property_type"]
                                      == top_chart_property_type]
        ...
        price_df = pd.DataFrame(top_chart_df_1_sorted.groupby(
            "host_neighbourhood")["price"].agg(["sum", "mean"]))
        price_df.reset_index(inplace=True)
        price_df.columns = ["host_neighbourhood",
                            "Total_price", "Avarage_price"]
        ...
        with col1:
            ...
            price_df_1 = pd.DataFrame(top_chart_df_1_sorted.groupby(
                "host_location")["price"].agg(["sum", "mean"]))
            price_df_1.reset_index(inplace=True)
            price_df_1.columns = ["host_location",
                                  "Total_price", "Avarage_price"]
        ...
        room_type_t = st.selectbox(
            "Select the Room Type for chart view", top_chart_df_1_sorted["room_type"].unique())
        ...
        fig_top_50_price_1 = px.bar(top_chart_df_2op_50_price, x="name",  y="price", color="price",
                                    ...
                                    range_color=(
                                        0, top_chart_df_2op_50_price["price"].max()),
...
if option == 'About':
    ...
    st.header(":violet[ABOUT AIRBNB PROJECT]")
